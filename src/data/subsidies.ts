export type AgeGroup = '10대' | '20대' | '30대' | '40대' | '50대' | '60대' | '70대이상';
export type Gender = '전체' | '남성' | '여성';
export type Category = '주거' | '취업·창업' | '금융' | '복지·돌봄' | '교육' | '건강' | '문화·여가';
/** '전국'은 어디 거주해도 해당되는 국가 사업. 나머지는 해당 지자체 거주자 전용 사업입니다. */
export type Region = '전국' | '서울' | '부산' | '경기' | '인천';

export interface Subsidy {
  id: number;
  title: string;
  description: string;
  amount: string;
  deadline: string;
  regions: Region[];
  category: Category;
  ageGroups: AgeGroup[];
  genders: Gender[];
  source: string;
  url: string;
  isUrgent?: boolean;
}

/** 이 데이터의 마지막 콘텐츠 갱신일(신규 항목 추가·금액/조건 재점검 기준일) */
export const LAST_UPDATED = '2026-09-01';

export const subsidies: Subsidy[] = [
  // ── 주거 ──
  {
    id: 1,
    title: '청년 월세 특별지원',
    description: '만 19~34세 독립 거주 무주택 청년에게 월세 최대 20만원을 최대 24개월 지원합니다. 2026년 한시 사업이 종료되고 상시 사업으로 전환되어, 자격을 충족하면 연중 언제든 신청할 수 있습니다.',
    amount: '월 최대 20만원 (최대 24개월, 총 480만원)',
    deadline: '상시',
    regions: ['전국'],
    category: '주거',
    ageGroups: ['20대', '30대'],
    genders: ['전체'],
    source: '국토교통부',
    url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00004661',
  },
  {
    id: 2,
    title: '청년전용 버팀목 전세자금대출',
    description: '만 19~34세 무주택 청년에게 전세자금을 연 1.7~2.7% 저금리로 최대 2억원까지 대출 지원합니다.',
    amount: '최대 2억원 (연 1.7~2.7% 저금리)',
    deadline: '상시',
    regions: ['전국'],
    category: '주거',
    ageGroups: ['20대', '30대'],
    genders: ['전체'],
    source: '국토교통부 (주택도시기금)',
    url: 'https://www.myhome.go.kr/hws/portal/cont/selectYouthPolicyYouthOnlyCrutchLoanView.do',
  },
  {
    id: 3,
    title: '청년 전세보증금 반환보증 보증료 지원',
    description: '만 34세 이하 청년 세입자의 전세보증금 반환보증 보증료를 최대 30만원 지원합니다.',
    amount: '최대 30만원',
    deadline: '상시',
    regions: ['전국'],
    category: '주거',
    ageGroups: ['20대', '30대'],
    genders: ['전체'],
    source: '국토교통부',
    url: 'https://www.hf.go.kr',
  },
  {
    id: 4,
    title: '주거급여 (노인·저소득층)',
    description: '기초생활수급자 및 차상위계층에게 임차료 또는 주택 수선 비용을 지원합니다. 65세 이상 어르신 포함.',
    amount: '월 최대 33만원',
    deadline: '상시',
    regions: ['전국'],
    category: '주거',
    ageGroups: ['40대', '50대', '60대', '70대이상'],
    genders: ['전체'],
    source: '국토교통부',
    url: 'https://www.myhome.go.kr',
  },

  // ── 취업·창업 ──
  {
    id: 5,
    title: '국민취업지원제도 I유형',
    description: '취업 취약계층에게 월 60만원의 구직촉진수당을 최대 6개월 지급하고 취업 지원 서비스를 제공합니다. 2026년 월 50만원→60만원으로 인상.',
    amount: '월 60만원 × 최대 6개월 (총 360만원)',
    deadline: '상시',
    regions: ['전국'],
    category: '취업·창업',
    ageGroups: ['20대', '30대', '40대', '50대'],
    genders: ['전체'],
    source: '고용노동부',
    url: 'https://www.work24.go.kr/ua/z/z/1300/selectEmssRqutIntro.do',
  },
  {
    id: 6,
    title: '국민취업지원제도 II유형',
    description: '취업 지원 서비스와 함께 직업훈련 참여 시 월 최대 28만 4천원의 취업활동비용을 지원합니다.',
    amount: '월 최대 28.4만원 (취업활동비)',
    deadline: '상시',
    regions: ['전국'],
    category: '취업·창업',
    ageGroups: ['20대', '30대', '40대', '50대', '60대'],
    genders: ['전체'],
    source: '고용노동부',
    url: 'https://www.work24.go.kr/ua/z/z/1300/selectEmssRqutIntro.do',
  },
  {
    id: 7,
    title: '청년도전지원사업',
    description: '6개월 이상 구직단념 청년(만 18~34세)에게 맞춤형 프로그램 및 참여수당 등 최대 350만원을 지원합니다.',
    amount: '최대 350만원 (참여수당+인센티브)',
    deadline: '상시',
    regions: ['전국'],
    category: '취업·창업',
    ageGroups: ['10대', '20대', '30대'],
    genders: ['전체'],
    source: '고용노동부',
    url: 'https://www.work.go.kr/youngChallenge/index.do',
  },
  {
    id: 8,
    title: '여성 새로일하기센터 취업지원',
    description: '경력단절 여성에게 직업상담, 직업훈련, 인턴십, 취업장려금 등 종합 취업 지원 서비스를 제공합니다.',
    amount: '훈련비 지원 + 취업장려금',
    deadline: '상시',
    regions: ['전국'],
    category: '취업·창업',
    ageGroups: ['20대', '30대', '40대', '50대'],
    genders: ['여성'],
    source: '여성가족부',
    url: 'https://www.saeil.mogef.go.kr',
  },
  {
    id: 9,
    title: '예비창업패키지',
    description: '생애 첫 창업 청년을 위해 사업화 자금(평균 4,700만원, 최대 1억원), 멘토링, 교육을 지원합니다.',
    amount: '평균 4,700만원 (최대 1억원)',
    deadline: '상시',
    regions: ['전국'],
    category: '취업·창업',
    ageGroups: ['20대', '30대'],
    genders: ['전체'],
    source: '중소벤처기업부 (창업진흥원)',
    url: 'https://www.kised.or.kr/menu.es?mid=a10205010000',
  },
  {
    id: 10,
    title: '초기창업패키지',
    description: '창업 3년 이내 청년 창업자에게 사업화 자금 최대 1억원 및 전담 멘토링을 지원합니다.',
    amount: '최대 1억원',
    deadline: '상시',
    regions: ['전국'],
    category: '취업·창업',
    ageGroups: ['20대', '30대', '40대'],
    genders: ['전체'],
    source: '중소벤처기업부 (창업진흥원)',
    url: 'https://www.kised.or.kr/menu.es?mid=a10205020000',
  },
  {
    id: 11,
    title: '중장년 취업아카데미',
    description: '만 40~65세 중장년층을 대상으로 직업훈련과 취업 알선을 무료로 제공합니다.',
    amount: '무료 훈련 제공',
    deadline: '상시',
    regions: ['전국'],
    category: '취업·창업',
    ageGroups: ['40대', '50대', '60대'],
    genders: ['전체'],
    source: '고용노동부',
    url: 'https://www.work.go.kr',
  },

  // ── 금융 ──
  {
    id: 12,
    title: '청년미래적금',
    description: '만 19~34세 청년이 3년간 매월 최대 50만원 납입 시 정부 기여금(일반형 6%·우대형 12%)과 비과세 혜택으로 목돈 마련 가능. 1차 모집(6~7월)은 마감되었으며, 2차 모집은 2026년 12월경 예정입니다(정확한 일정은 취급 은행 공지 확인).',
    amount: '월 최대 50만원 납입 (정부기여금 6~12%, 3년 만기)',
    deadline: '2026년 12월경 (2차 모집 예정)',
    regions: ['전국'],
    category: '금융',
    ageGroups: ['20대', '30대'],
    genders: ['전체'],
    source: '금융위원회',
    url: 'https://lifeinfos.com/%EC%B2%AD%EB%85%84%EB%AF%B8%EB%9E%98%EC%A0%81%EA%B8%88-%EC%B4%9D%EC%A0%95%EB%A6%AC/',
  },
  {
    id: 13,
    title: '청년도약계좌 (신규가입 종료·기존 가입자 유지)',
    description: '만 19~34세 청년 대상 5년 만기 적금으로 정부 기여금과 비과세 혜택을 제공했으나, 신규 가입은 2025년 12월 31일부로 종료되었습니다. 기존 가입자는 만기까지 혜택이 그대로 유지되며, 신규 가입을 원하면 청년미래적금(위 항목)을 확인하세요.',
    amount: '최대 5,000만원 (5년 만기, 기존 가입자 한정)',
    deadline: '신규가입 마감(2025-12-31), 기존 가입자는 만기까지 유지',
    regions: ['전국'],
    category: '금융',
    ageGroups: ['20대', '30대'],
    genders: ['전체'],
    source: '금융위원회',
    url: 'https://ylaccount.kinfa.or.kr/',
  },
  {
    id: 14,
    title: '근로장려금 (EITC)',
    description: '저소득 근로자·사업자 가구에 근로 의욕을 높이기 위해 장려금을 지급합니다. 2026년 맞벌이 소득 기준 4,400만원으로 상향. 정기신청분은 5월 신청 후 9월 말(추석 전) 지급되며, 근로소득만 있는 가구는 반기신청도 가능해 상반기 소득분은 9월 1~15일, 하반기 소득분은 이듬해 3월에 홈택스·손택스로 신청하면 산정액의 35%를 앞당겨 받습니다.',
    amount: '단독 최대 165만원 / 홑벌이 285만원 / 맞벌이 330만원',
    deadline: '정기신청 5월 · 반기신청 3월·9월(1~15일)',
    regions: ['전국'],
    category: '금융',
    ageGroups: ['20대', '30대', '40대', '50대', '60대', '70대이상'],
    genders: ['전체'],
    source: '국세청',
    url: 'https://hometax.go.kr',
  },
  {
    id: 15,
    title: '자녀장려금',
    description: '18세 미만 자녀를 양육하는 저소득 가구에 자녀 1인당 최대 100만원의 장려금을 지급합니다.',
    amount: '자녀 1인당 최대 100만원',
    deadline: '상시 (정기신청 매년 5월)',
    regions: ['전국'],
    category: '금융',
    ageGroups: ['20대', '30대', '40대', '50대'],
    genders: ['전체'],
    source: '국세청',
    url: 'https://hometax.go.kr',
  },
  {
    id: 16,
    title: '소상공인 정책자금 대출',
    description: '소상공인의 사업 운전·시설자금을 저금리(연 2~3%)로 최대 7천만원까지 지원합니다.',
    amount: '최대 7,000만원 (연 2~3% 저금리)',
    deadline: '상시',
    regions: ['전국'],
    category: '금융',
    ageGroups: ['20대', '30대', '40대', '50대', '60대'],
    genders: ['전체'],
    source: '소상공인시장진흥공단',
    url: 'https://www.semas.or.kr',
  },

  // ── 복지·돌봄 ──
  {
    id: 17,
    title: '기초연금',
    description: '만 65세 이상 소득 하위 70% 어르신에게 매월 최대 34만 9,700원을 지급합니다. 2026년 선정기준액은 단독가구 247만원, 부부가구 395만 2천원.',
    amount: '월 최대 34만 9,700원',
    deadline: '상시',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['60대', '70대이상'],
    genders: ['전체'],
    source: '보건복지부',
    url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00001164',
  },
  {
    id: 18,
    title: '부모급여',
    description: '만 0~23개월 아동 양육 가구에 지급. 0~11개월 월 100만원, 12~23개월 월 50만원 지원(소득무관).',
    amount: '월 50~100만원 (총 최대 1,800만원)',
    deadline: '상시',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['20대', '30대', '40대'],
    genders: ['전체'],
    source: '보건복지부',
    url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00004657',
  },
  {
    id: 19,
    title: '첫만남이용권',
    description: '출생 직후 첫째 200만원, 둘째 이상 300만원을 국민행복카드 바우처로 지급합니다(소득무관).',
    amount: '첫째 200만원 / 둘째 이상 300만원',
    deadline: '상시',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['20대', '30대', '40대'],
    genders: ['전체'],
    source: '보건복지부',
    url: 'https://welfare-mom.com/korea-child-benefit-guide-2026/',
  },
  {
    id: 20,
    title: '아동수당',
    description: '2026년부터 만 9세 미만(0~107개월) 아동에게 월 10만원(비수도권 최대 12만원)을 지급합니다.',
    amount: '월 10~12만원',
    deadline: '상시',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['20대', '30대', '40대'],
    genders: ['전체'],
    source: '보건복지부',
    url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00001171',
  },
  {
    id: 21,
    title: '한부모가족 아동양육비',
    description: '기준 중위소득 65% 이하 한부모가족 만 18세 미만 아동 1인당 월 양육비를 지원합니다.',
    amount: '아동 1인당 월 양육비 지원',
    deadline: '상시',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['20대', '30대', '40대'],
    genders: ['전체'],
    source: '여성가족부',
    url: 'https://www.mogef.go.kr/io/ind/io_ind_s005d.do?mid=old919&bbtSn=15',
  },
  {
    id: 22,
    title: '장애인연금',
    description: '18세 이상 중증장애인 중 소득 하위 70%에게 매월 최대 43만 9,700원(기초급여+부가급여)을 지급합니다.',
    amount: '월 최대 43만 9,700원',
    deadline: '상시',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['10대', '20대', '30대', '40대', '50대', '60대', '70대이상'],
    genders: ['전체'],
    source: '보건복지부',
    url: 'https://www.bokjiro.go.kr/pension',
  },
  {
    id: 23,
    title: '노인일자리 및 사회활동 지원',
    description: '2026년 역대 최대 115만 2천 개 일자리 제공. 만 60세 이상 어르신에게 월 최대 76만원의 활동비를 지원합니다.',
    amount: '월 최대 76만원',
    deadline: '상시',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['60대', '70대이상'],
    genders: ['전체'],
    source: '보건복지부',
    url: 'https://www.mohw.go.kr/menu.es?mid=a10712020100',
  },
  {
    id: 24,
    title: '노인 장기요양보험',
    description: '신체 활동 및 가사 지원이 필요한 65세 이상 어르신에게 등급 판정 후 요양서비스 비용의 최대 85%를 지원합니다.',
    amount: '서비스 비용의 85% 지원',
    deadline: '상시',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['60대', '70대이상'],
    genders: ['전체'],
    source: '국민건강보험공단',
    url: 'https://www.nhis.or.kr',
  },
  {
    id: 25,
    title: '기초생활보장 생계급여',
    description: '기준 중위소득 32% 이하 가구에 최저 생계비를 지급합니다. 2026년 1인가구 기준 월 82만 556원.',
    amount: '1인가구 월 82만원 / 2인가구 134만원',
    deadline: '상시',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['10대', '20대', '30대', '40대', '50대', '60대', '70대이상'],
    genders: ['전체'],
    source: '보건복지부',
    url: 'https://www.mohw.go.kr/menu.es?mid=a10708010300',
  },
  {
    id: 26,
    title: '임신·출산 진료비 지원 (국민행복카드)',
    description: '임신 확인 후 단태아 100만원, 다태아 140만원의 진료비 바우처를 지원합니다.',
    amount: '단태아 100만원 / 다태아 140만원',
    deadline: '상시',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['20대', '30대', '40대'],
    genders: ['여성'],
    source: '보건복지부',
    url: 'https://www.nhis.or.kr',
  },

  // ── 교육 ──
  {
    id: 27,
    title: '국가장학금 (I·II 유형)',
    description: '소득 9구간 이하 대학생에게 소득분위별 연간 최대 570만원의 등록금을 지원합니다. 2026년 지원 대상 9구간으로 확대.',
    amount: '연 최대 570만원 (기초·차상위 전액)',
    deadline: '상시 (학기별 신청)',
    regions: ['전국'],
    category: '교육',
    ageGroups: ['10대', '20대'],
    genders: ['전체'],
    source: '한국장학재단',
    url: 'https://www.kosaf.go.kr/',
  },
  {
    id: 28,
    title: '국민내일배움카드',
    description: '만 15세 이상 재직자·구직자 누구나 5년간 300~500만원 한도 훈련비 바우처를 통해 직업훈련을 받을 수 있습니다.',
    amount: '300~500만원 (바우처, 5년)',
    deadline: '상시',
    regions: ['전국'],
    category: '교육',
    ageGroups: ['10대', '20대', '30대', '40대', '50대', '60대', '70대이상'],
    genders: ['전체'],
    source: '고용노동부',
    url: 'https://www.hrd.go.kr',
  },
  {
    id: 29,
    title: 'K-디지털 트레이닝',
    description: 'AI·빅데이터·클라우드 등 신기술 분야 집중 훈련 과정에 최대 200만원의 훈련비를 지원합니다.',
    amount: '최대 200만원',
    deadline: '상시',
    regions: ['전국'],
    category: '교육',
    ageGroups: ['20대', '30대', '40대'],
    genders: ['전체'],
    source: '고용노동부',
    url: 'https://www.hrd.go.kr',
  },
  {
    id: 30,
    title: '청년창업 세액감면',
    description: '만 15~34세 청년이 창업한 경우 최초 소득 발생 연도부터 최대 5년간 소득세·법인세 50% 이상 감면 혜택을 받습니다.',
    amount: '소득세·법인세 50~100% 감면 (최대 5년)',
    deadline: '상시',
    regions: ['전국'],
    category: '교육',
    ageGroups: ['10대', '20대', '30대'],
    genders: ['전체'],
    source: '중소벤처기업부',
    url: 'https://www.k-startup.go.kr/',
  },

  // ── 건강 ──
  {
    id: 31,
    title: '정신건강 심리상담 바우처',
    description: '우울·불안 등 정서적 어려움이 있는 국민 누구나 전문 심리상담 8회를 무료 또는 낮은 본인부담으로 지원받습니다.',
    amount: '8회 심리상담 (무료 또는 소액 자부담)',
    deadline: '상시 (예산 소진 시 조기마감)',
    regions: ['전국'],
    category: '건강',
    ageGroups: ['10대', '20대', '30대', '40대', '50대', '60대', '70대이상'],
    genders: ['전체'],
    source: '보건복지부',
    url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00005567',
  },
  {
    id: 32,
    title: '일반건강검진 (국가건강검진)',
    description: '2026년은 짝수년으로 짝수 출생연도 해당자 대상. 2년마다 기본 건강검진 무료 제공, 40세·66세 생애전환기 검진 추가.',
    amount: '무료',
    deadline: '2026-12-31',
    regions: ['전국'],
    category: '건강',
    ageGroups: ['20대', '30대', '40대', '50대', '60대', '70대이상'],
    genders: ['전체'],
    source: '국민건강보험공단',
    url: 'https://www.nhis.or.kr/nhis/healthin/wbhaca04500m01.do',
    isUrgent: true,
  },
  {
    id: 33,
    title: '국가암검진 지원',
    description: '의료급여수급자 및 건강보험료 하위 50% 가구 대상 위암(40세↑)·대장암(50세↑)·간암·폐암·유방암·자궁경부암 무료 검진.',
    amount: '무료 (건강보험 하위 50%) 또는 본인부담 10%',
    deadline: '상시',
    regions: ['전국'],
    category: '건강',
    ageGroups: ['40대', '50대', '60대', '70대이상'],
    genders: ['전체'],
    source: '국민건강보험공단',
    url: 'https://www.cancer.go.kr/lay1/S1T549C553/sublink.do',
  },
  {
    id: 34,
    title: '여성 암 검진 지원',
    description: '자궁경부암(20세↑)·유방암(40세↑) 정기 검진을 건강보험 적용 또는 무료로 지원합니다.',
    amount: '무료 또는 검진비 90% 지원',
    deadline: '상시',
    regions: ['전국'],
    category: '건강',
    ageGroups: ['20대', '30대', '40대', '50대', '60대', '70대이상'],
    genders: ['여성'],
    source: '국민건강보험공단',
    url: 'https://www.nhis.or.kr',
  },

  // ── 문화·여가 ──
  {
    id: 35,
    title: '문화누리카드',
    description: '기초수급자 및 차상위계층에게 공연·영화·스포츠·여행 등 문화생활비로 연간 15만원(청소년 16만원)을 지원합니다.',
    amount: '연 15만원 (청소년 16만원)',
    deadline: '상시',
    regions: ['전국'],
    category: '문화·여가',
    ageGroups: ['10대', '20대', '30대', '40대', '50대', '60대', '70대이상'],
    genders: ['전체'],
    source: '문화체육관광부',
    url: 'https://www.mnuri.kr/munhwa/cardIssueGuide.do',
  },
  {
    id: 36,
    title: '어르신 문화프로그램',
    description: '60세 이상 어르신을 위한 문화예술 교육프로그램을 전국 문화원·복지관에서 무료로 제공합니다.',
    amount: '무료',
    deadline: '상시',
    regions: ['전국'],
    category: '문화·여가',
    ageGroups: ['60대', '70대이상'],
    genders: ['전체'],
    source: '문화체육관광부',
    url: 'https://www.culture.go.kr',
  },
  {
    id: 37,
    title: '청소년 문화·여가 활동 지원',
    description: '저소득 가정 청소년에게 문화·예술·체육 활동 참여 비용을 바우처 형태로 지원합니다.',
    amount: '연 최대 40만원 (바우처)',
    deadline: '상시',
    regions: ['전국'],
    category: '문화·여가',
    ageGroups: ['10대'],
    genders: ['전체'],
    source: '여성가족부',
    url: 'https://www.youth.go.kr',
  },

  // ── 2026-08-19 추가 ──
  {
    id: 38,
    title: '신생아 특례대출',
    description: '대출 접수일 기준 2년 이내 출산(2023년 1월 1일 이후 출생아부터 적용)한 가구에 주택 구입·전세자금을 저금리로 지원합니다. 부부합산 연소득 2억원 이하(맞벌이 기준) 등 소득 요건이 있습니다.',
    amount: '구입자금 최대 4억원 / 전세자금 최대 2억 4천만원',
    deadline: '상시',
    regions: ['전국'],
    category: '주거',
    ageGroups: ['20대', '30대', '40대'],
    genders: ['전체'],
    source: '국토교통부',
    url: 'https://www.hf.go.kr',
  },
  {
    id: 39,
    title: '청년내일저축계좌',
    description: '만 15~39세 청년이 매월 10만원을 저축하면 정부가 월 30만원을 함께 적립해줍니다. 가구소득 기준 중위소득 50% 이하이고 월 10만원 이상 근로·사업소득이 있어야 신청 가능. 2026년 신청기간은 5월 4~20일이었습니다(연 1회 모집, 다음 회차 일정은 복지로 공지 확인).',
    amount: '3년 만기 시 총 1,440만원 이상 수령(본인납입 360만원+정부적립 1,080만원+이자)',
    deadline: '상시 (연 1회 모집, 정확한 회차 일정은 복지로 확인)',
    regions: ['전국'],
    category: '금융',
    ageGroups: ['10대', '20대', '30대'],
    genders: ['전체'],
    source: '보건복지부',
    url: 'https://www.gov.kr/portal/rcvfvrSvc/dtlEx/135200005013',
  },
  {
    id: 40,
    title: '평생교육이용권(평생교육바우처)',
    description: '2026년부터 일반형 소득기준이 폐지되어 만 19세 이상이면 누구나 온라인 강좌·학원 등 평생교육비를 지원받을 수 있습니다(기초생활수급자·차상위·한부모가족 우선 선정). 디지털형(30세 이상)·어르신형(65세 이상)·장애인형이 별도로 운영되며, 2026년부터 시·도별 직접 운영으로 바뀌어 신청기간이 지역마다 다릅니다.',
    amount: '연 35만원 (적극 수강 시 최대 70만원)',
    deadline: '연 2회 모집 (상·하반기, 지자체별 상이)',
    regions: ['전국'],
    category: '교육',
    ageGroups: ['20대', '30대', '40대', '50대', '60대', '70대이상'],
    genders: ['전체'],
    source: '국가평생교육진흥원',
    url: 'https://www.class.moe.go.kr',
  },
  {
    id: 41,
    title: '노인맞춤돌봄서비스',
    description: '65세 이상 기초생활수급자·차상위계층·기초연금수급자 중 독거·고령부부 가구 등 돌봄이 필요한 어르신에게 안부확인, 가사지원, 병원 동행, 문화활동 등을 제공합니다. 2026년부터 AI 센서·스마트워치 활용 디지털 돌봄이 강화되었습니다.',
    amount: '방문·안전확인·생활교육 등 서비스 제공(본인부담 없음)',
    deadline: '상시',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['60대', '70대이상'],
    genders: ['전체'],
    source: '보건복지부',
    url: 'https://www.bokjiro.go.kr',
  },
  {
    id: 42,
    title: '저소득층 기저귀·조제분유 지원',
    description: '기초생활수급자·차상위·한부모가족의 만 0~24개월 영아에게 기저귀·조제분유 비용을 국민행복카드 바우처로 지원합니다. 2026년 7월부터 장애인 가구·2자녀 이상 다자녀 가구는 소득기준이 중위소득 100% 이하로 확대되었습니다.',
    amount: '월 최대 20만원 (기저귀+조제분유 병행 시)',
    deadline: '상시 (출생 후 만 2세 전날까지 신청)',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['20대', '30대', '40대'],
    genders: ['전체'],
    source: '보건복지부',
    url: 'https://www.bokjiro.go.kr',
  },
  {
    id: 43,
    title: '산모·신생아 건강관리 지원 (산후도우미)',
    description: '가구의 건강보험료 본인부담금 합산액이 기준 중위소득 150% 이하인 출산가정에 산모·신생아 건강관리사(산후도우미) 서비스 비용을 지원합니다. 지원금액은 태아 유형·출산순위·소득수준·서비스기간에 따라 차등 산정됩니다.',
    amount: '소득·서비스 유형별 차등 지원(바우처, 정확한 금액은 관할 보건소 확인)',
    deadline: '상시',
    regions: ['전국'],
    category: '복지·돌봄',
    ageGroups: ['20대', '30대', '40대'],
    genders: ['여성'],
    source: '보건복지부',
    url: 'https://www.bokjiro.go.kr',
  },

  // ── 지자체별 지원금 (2026-08-19 추가) ──
  {
    id: 44,
    title: '서울시 청년 기후동행카드 할인',
    description: '서울 거주 여부와 무관하게 만 19~39세 청년이면 기후동행카드를 청년 할인가로 이용할 수 있습니다. 일반 요금 대비 매달 7,000원, 연간 최대 84,000원을 아낄 수 있습니다.',
    amount: '월 55,000원(따릉이 미포함)·58,000원(포함) — 일반가 대비 월 7,000원 할인',
    deadline: '상시',
    regions: ['서울'],
    category: '복지·돌봄',
    ageGroups: ['20대', '30대'],
    genders: ['전체'],
    source: '서울특별시',
    url: 'https://news.seoul.go.kr/traffic/archives/510651',
  },
  {
    id: 45,
    title: '부산청년 기쁨두배통장',
    description: '부산시에 주민등록을 두고 4대보험 중 하나 이상에 가입해 일하는 만 18~39세 청년이 매월 10만원씩 저축하면 부산시가 동일 금액을 추가로 적립해줍니다. 기준 중위소득 150% 이하 등 소득 요건이 있습니다.',
    amount: '24개월형 480만원+이자 / 36개월형 720만원+이자',
    deadline: '상시 (연 1회 모집, 정확한 공고 일정은 부산청년플랫폼 확인)',
    regions: ['부산'],
    category: '금융',
    ageGroups: ['20대', '30대'],
    genders: ['전체'],
    source: '부산광역시',
    url: 'https://young.busan.go.kr/index.nm?menuCd=53',
  },
  {
    id: 46,
    title: '경기도 청년기본소득',
    description: '경기도에 최근 3년 이상 계속 거주했거나 합산 10년 이상 거주한 만 24세 청년에게 소득·재산·취업 여부와 무관하게 분기별로 지역화폐를 지급합니다. 신청 대상 출생연도는 분기마다 바뀝니다.',
    amount: '분기 25만원 (연 최대 100만원, 지역화폐)',
    deadline: '상시 (분기별 신청, 정확한 신청기간은 경기청년포털 확인)',
    regions: ['경기'],
    category: '금융',
    ageGroups: ['20대'],
    genders: ['전체'],
    source: '경기도',
    url: 'https://www.gg.go.kr',
  },
  {
    id: 47,
    title: '인천형 아이 꿈 수당',
    description: '인천 거주 아동에게 만 8세부터 18세까지 단계적으로 지원 연령을 확대하는 아동수당입니다. 2026년 기준 출생연도별로 월 5만원 또는 10만원이 지급되며, 매년 지원 대상 연령이 한 살씩 늘어납니다.',
    amount: '월 5만원 또는 10만원 (출생연도별 상이)',
    deadline: '상시',
    regions: ['인천'],
    category: '복지·돌봄',
    ageGroups: ['20대', '30대', '40대'],
    genders: ['전체'],
    source: '인천광역시',
    url: 'https://www.incheon.go.kr/welfare/WE020409',
  },
];

export const AGE_GROUPS: AgeGroup[] = ['10대', '20대', '30대', '40대', '50대', '60대', '70대이상'];
export const GENDERS: Gender[] = ['전체', '남성', '여성'];
export const REGIONS: Region[] = ['전국', '서울', '부산', '경기', '인천'];
export const CATEGORIES: Category[] = ['주거', '취업·창업', '금융', '복지·돌봄', '교육', '건강', '문화·여가'];

export const CATEGORY_EMOJI: Record<Category, string> = {
  '주거': '🏠',
  '취업·창업': '💼',
  '금융': '💰',
  '복지·돌봄': '🤝',
  '교육': '📚',
  '건강': '❤️',
  '문화·여가': '🎭',
};

export function filterSubsidies(
  ageGroup: AgeGroup | null,
  gender: Gender,
): Subsidy[] {
  return subsidies.filter((s) => {
    const ageMatch = ageGroup == null || s.ageGroups.includes(ageGroup);
    const genderMatch =
      gender === '전체' ||
      s.genders.includes('전체') ||
      s.genders.includes(gender);
    return ageMatch && genderMatch;
  });
}
